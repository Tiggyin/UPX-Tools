...
struct AppConfig {
    ...
    #[serde(default = "default_upx_path")]
    upx_path: String,
}
...
fn get_upx_path(config: &AppConfig) -> Option<PathBuf> {
    // 1. 用户自定义路径
    if !config.upx_path.is_empty() {
        let custom_upx = PathBuf::from(&config.upx_path);
        if custom_upx.exists() {
            return Some(custom_upx);
        }
    }
...
#[tauri::command]
async fn process_upx(options: UpxOptions, config: AppConfig) -> Result<String, String> {
    let upx_path = get_upx_path(&config).ok_or("未找到 UPX 工具！请确保安装完整")?;
...
#[tauri::command]
fn check_path_type(path: String) -> Result<PathInfo, String> {
    ...
    if path_obj.is_dir() {
        ...
        let metadata = fs::metadata(path_obj).map_err(|e| format!("无法读取文件属性: {}", e))?;
        ...
        if let Some(ext) = path_obj.extension() {
            ...
            if ext_lower == "exe" || ext_lower == "dll" {
                // 检查是否已加壳
                let is_packed = check_if_packed(path_obj, &size);
                let upx_version = if is_packed {
                    get_upx_version_from_file(path_obj).ok()
                } else {
                    None
                };
    ...
    if let Ok(mut file) = fs::File::open(file_path) {
        ...
        if let Ok(bytes_read) = file.read(&mut buffer) {
            if bytes_read >= 4 {
                // UPX 的特征签名
                let upx_signatures = [
                    [0x50, 0x4B, 0x03, 0x04], // ZIP 签名（UPX 可能使用）
                    [0x55, 0x50, 0x58, 0x01], // UPX v1 签名
                    [0x55, 0x50, 0x58, 0x02], // UPX v2 签名
                    [0x55, 0x50, 0x58, 0x03], // UPX v3 签名
                ];
...
// 从文件获取 UPX 版本信息
fn get_upx_version_from_file(_file_path: &Path) -> Result<String, String> {
    // 这里可以实现更复杂的版本检测逻辑
    // 现在返回一个固定值
...
#[tauri::command]
fn get_upx_version(config: AppConfig) -> Result<String, String> {
    let upx_path = get_upx_path(&config).ok_or("未找到 UPX 工具！")?;
