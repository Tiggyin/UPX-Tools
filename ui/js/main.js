...
function initDOMElements() {
    ...
    upxPathInput = $('upx-path-input')
    browseUpxBtn = $('browse-upx-btn')
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const config = {
            ...
            upx_path: upxPathInput.value.trim(),
        }

        const version = await invoke('get_upx_version', config)
...
function initOperationButtons() {
    ...
    browseUpxBtn.addEventListener('click', handleBrowseUpx)
...
// 处理 UPX 路径选择
async function handleBrowseUpx() {
    try {
        const selected = await open({
            multiple: false,
            filters: [{ name: 'UPX 可执行文件', extensions: ['exe'] }],
        })

        if (selected) {
            upxPathInput.value = selected
            addLog(`已选择 UPX 路径: ${selected}`, 'info')
        }
    } catch (error) {
        addLog(`选择 UPX 路径失败: ${error}`, 'error')
    }
}
...
async function checkAndScanPath(path) {
    const pathObj = await invoke('check_path_type', { path })
    ...
    if (pathObj.type === 'folder') {
        ...
        if (files.length > 0) {
            ...
            addLog(`文件夹中没有找到可执行的 .exe 或 .dll 文件`, 'warning')
            return []
        }
    } else if (pathObj.type === 'file') {
        if (pathObj.extension === 'exe' || pathObj.extension === 'dll') {
            addLog(`检测到文件: ${path}`, 'info')
            addLog(`文件名: ${pathObj.name}`, 'info')
            addLog(`文件大小: ${formatBytes(pathObj.size)}`, 'info')
            addLog(`是否已加壳: ${pathObj.is_packed ? '是' : '否'}`, 'info')
            if (pathObj.is_packed) {
                addLog(`UPX 版本: ${pathObj.upx_version || '未知'}`, 'info')
            }
            return [path]
        } else {
            addLog(`不支持的文件类型: ${path}`, 'warning')
            return []
        }
    }
...
async function processUpx(mode, inputFile, outputFile) {
    try {
        ...
        const result = await invoke('process_upx', { options, config })
...
async function handleCopyLog() {
    try {
        ...
        // 3秒后隐藏复制按钮
        setTimeout(hideCopyLogButton, 3000)
    } catch (error) {
...
// 显示/隐藏复制按钮
function showCopyLogButton() {
    if (copyLogBtn) {
        copyLogBtn.classList.remove('opacity-0', 'pointer-events-none')
        copyLogBtn.classList.add('opacity-100')
    }
}

function hideCopyLogButton() {
    if (copyLogBtn) {
        copyLogBtn.classList.remove('opacity-100')
        copyLogBtn.classList.add('opacity-0', 'pointer-events-none')
    }
}
...
function applyConfigToUI(config) {
    ...
    upxPathInput.value = config.upx_path || ''
